...
function HomeScreen({ onStart }) {
  ...
  return (
    <div style={{ ...S.page, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "20px", position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ position: "absolute", top: "5%", left: "10%", width: 400, height: 400, borderRadius: "50%", background: "radial-gradient(circle,rgba(0,212,255,.07) 0%,transparent 70%)", pointerEvents: "none" }} />
      <div style={{ position: "absolute", bottom: "5%", right: "5%", width: 300, height: 300, borderRadius: "50%", background: "radial-gradient(circle,rgba(255,107,107,.07) 0%,transparent 70%)", pointerEvents: "none" }} />
      ...
      <div style={{ position: "relative", zIndex: 1, width: "100%", maxWidth: 820 }}>
        <div style={{ textAlign: "center", marginBottom: 40 }}>
          ...
          <p style={{ fontSize: "clamp(13px,2vw,18px)", color: "rgba(255,255,255,.4)", letterSpacing: 6, margin: 0 }}>NATIONAL QUALIFIER TEST</p>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 12, marginBottom: 36 }}>
          {[
            ...
            <div key={i.l} style={{ ...S.card, textAlign: "center" }}>
              ...
              <div style={{ fontSize: 10, color: "rgba(255,255,255,.35)", letterSpacing: 2, marginTop: 4 }}>{i.l.toUpperCase()}</div>
            </div>
          ))}
        </div>
        ...
        <div style={{ display: "flex", gap: 0, marginBottom: 28, borderBottom: "1px solid rgba(255,255,255,0.08)" }}>
          {["register", "leaderboard"].map((t) => (
            <button key={t} onClick={() => setTab(t)} style={{
              ...
              color: tab === t ? "#00D4FF" : "rgba(255,255,255,.35)",
        ...
        {tab === "register" ? (
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, maxWidth: 640, margin: "0 auto" }}>
            <div style={{ gridColumn: "1/-1" }}>
              <label style={{ fontSize: 10, color: "rgba(255,255,255,.4)", letterSpacing: 2, display: "block", marginBottom: 6 }}>FULL NAME *</label>
              <input style={S.input} placeholder="Your full name" value={name} onChange={(e) => setName(e.target.value)} />
            </div>
            <div>
              <label style={{ fontSize: 10, color: "rgba(255,255,255,.4)", letterSpacing: 2, display: "block", marginBottom: 6 }}>EMAIL *</label>
              <input style={S.input} placeholder="your@email.com" value={email} onChange={(e) => setEmail(e.target.value)} type="email" />
            </div>
            <div>
              <label style={{ fontSize: 10, color: "rgba(255,255,255,.4)", letterSpacing: 2, display: "block", marginBottom: 6 }}>PHONE *</label>
              <input style={S.input} placeholder="10-digit number" value={phone} onChange={(e) => setPhone(e.target.value)} type="tel" maxLength={10} />
            </div>
            <div style={{ gridColumn: "1/-1" }}>
              <label style={{ fontSize: 10, color: "rgba(255,255,255,.4)", letterSpacing: 2, display: "block", marginBottom: 6 }}>COLLEGE / UNIVERSITY</label>
            ...
            <div style={{ gridColumn: "1/-1", textAlign: "center", marginTop: 8 }}>
              ...
              <p style={{ fontSize: 10, color: "rgba(255,255,255,.2)", marginTop: 12, letterSpacing: 1 }}>
          ...
          <div style={{ maxWidth: 640, margin: "0 auto" }}>
            {loadingLB ? (
              <div style={{ textAlign: "center", color: "rgba(255,255,255,.3)", padding: 40 }}>Loading leaderboard...</div>
                  ...
                  return (
                    <div key={entry.id} style={{ ...S.card, display: "flex", alignItems: "center", gap: 16, borderLeft: `4px solid ${g.color}` }}>
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        <div style={{ fontSize: 11, color: "rgba(255,255,255,.35)" }}>{entry.college || "Student"}</div>
                      </div>
                      <div style={{ textAlign: "right" }}>
                        <div style={{ fontSize: 22, fontWeight: 700, color: g.color }}>{entry.score}%</div>
                        <div style={{ fontSize: 10, color: "rgba(255,255,255,.3)" }}>{entry.correct}/{entry.total} correct</div>
                      </div>
                    </div>
                  );
                })}
          ...
          {[
            ...
            <div key={item.t} style={{ ...S.card, display: "flex", gap: 16 }}>
              ...
              <div>
                ...
                <div style={{ fontSize: 12, color: "rgba(255,255,255,.45)", lineHeight: 1.6 }}>{item.d}</div>
              </div>
            </div>
          ))}
        </div>
        <div style={{ background: "rgba(0,212,255,0.06)", border: "1px solid rgba(0,212,255,0.2)", borderRadius: 8, padding: "14px 20px", marginBottom: 28, fontSize: 12, color: "rgba(0,212,255,.85)", lineHeight: 1.8 }}>
...
function TestScreen({ candidate, onComplete }) {
  ...
  const secResultsRef = useRef({});

  // keep refs in sync
  secIdxRef.current = secIdx;
  answersRef.current = answers;
  secResultsRef.current = secResults;

  const endSection = (currentSecIdx, currentAnswers, currentSecResults) => {
    clearInterval(timerRef.current);
    const sec = SECTIONS[currentSecIdx];
    let correct = 0, attempted = 0;
    sec.questions.forEach((q, i) => {
      const k = `${sec.id}-${i}`;
      if (currentAnswers[k] !== undefined) {
        attempted++;
        if (currentAnswers[k] === q.answer) correct++;
      }
    });
    const updated = { ...currentSecResults, [sec.id]: { correct, attempted, total: sec.questions.length } };
    secResultsRef.current = updated;
    setSecResults(updated);

    if (currentSecIdx < SECTIONS.length - 1) {
      setSecIdx(currentSecIdx + 1);
      setQIdx(0);
    } else {
      onComplete(updated);
    }
  ...
  useEffect(() => {
    ...
    timerRef.current = setInterval(() => {
      setTimeLeft((p) => {
        if (p <= 1) {
          clearInterval(timerRef.current);
          endSection(secIdxRef.current, answersRef.current, secResultsRef.current);
          return 0;
        }
        return p - 1;
      });
  ...
  return (
    <div style={{ ...S.page, display: "flex", flexDirection: "column" }}>
      {/* Top bar */}
      <div style={{ background: "#0d1117", borderBottom: "1px solid rgba(255,255,255,.08)", padding: "10px 20px", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 8 }}>
        <div>
          <div style={{ fontSize: 9, color: "rgba(255,255,255,.35)", letterSpacing: 2 }}>SECTION {secIdx + 1}/{SECTIONS.length}</div>
          <div style={{ fontSize: 13, fontWeight: 700, color: sec.color }}>{sec.name}</div>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 8, background: timeWarn ? "rgba(255,107,107,.1)" : "rgba(0,212,255,.06)", border: `1px solid ${timeWarn ? "rgba(255,107,107,.4)" : "rgba(0,212,255,.2)"}`, borderRadius: 6, padding: "7px 18px" }}>
          <span style={{ fontSize: 10, color: timeWarn ? "#FF6B6B" : "rgba(255,255,255,.4)", letterSpacing: 1 }}>⏱ TIME</span>
          <span style={{ fontSize: 22, fontWeight: 700, color: timeWarn ? "#FF6B6B" : "#00D4FF", fontVariantNumeric: "tabular-nums" }}>{fmt(timeLeft)}</span>
        </div>
        <div style={{ fontSize: 11, color: "rgba(255,255,255,.4)" }}>✅ {answered}/{sec.questions.length} &nbsp;|&nbsp; {candidate.name}</div>
      </div>
      ...
      <div style={{ display: "flex", flex: 1, overflow: "hidden" }}>
        ...
        <div style={{ flex: 1, padding: "28px 36px", overflowY: "auto" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
            <span style={{ fontSize: 10, color: "rgba(255,255,255,.3)", letterSpacing: 2 }}>Q{qIdx + 1} of {sec.questions.length} · {q.topic}</span>
            <button onClick={() => setFlagged((p) => ({ ...p, [key(qIdx)]: !p[key(qIdx)] }))}
              style={{ background: flagged[key(qIdx)] ? "rgba(255,107,107,.15)" : "transparent", border: `1px solid ${flagged[key(qIdx)] ? "#FF6B6B" : "rgba(255,255,255,.2)"}`, borderRadius: 4, padding: "5px 12px", color: flagged[key(qIdx)] ? "#FF6B6B" : "rgba(255,255,255,.35)", cursor: "pointer", fontSize: 10, letterSpacing: 1, fontFamily: "'IBM Plex Mono','Courier New',monospace" }}>
              {flagged[key(qIdx)] ? "🚩 FLAGGED" : "🏳 FLAG"}
            </button>
          </div>

          <div style={{ background: "rgba(255,255,255,.03)", border: "1px solid rgba(255,255,255,.08)", borderRadius: 10, padding: "24px 28px", marginBottom: 24, lineHeight: 1.9, fontSize: 15, whiteSpace: "pre-line" }}>
            {q.text}
          </div>
            ...
            {q.options.map((opt, i) => {
              ...
              return (
                <button key={i} onClick={() => setAnswers((p) => ({ ...p, [key(qIdx)]: i }))}
                  style={{ display: "flex", alignItems: "center", gap: 14, padding: "15px 22px", background: sel ? `${sec.color}15` : "rgba(255,255,255,.03)", border: `2px solid ${sel ? sec.color : "rgba(255,255,255,.08)"}`, borderRadius: 8, cursor: "pointer", textAlign: "left", color: "#fff", fontFamily: "'IBM Plex Mono','Courier New',monospace", fontSize: 14, lineHeight: 1.6, transition: "all .15s" }}>
                  <span style={{ width: 28, height: 28, borderRadius: 4, flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", background: sel ? sec.color : "rgba(255,255,255,.08)", color: sel ? "#000" : "rgba(255,255,255,.4)", fontWeight: 700, fontSize: 12 }}>{["A","B","C","D"][i]}</span>
                  {opt}
                </button>
              );
            })}
          ...
          <div style={{ display: "flex", justifyContent: "space-between", marginTop: 28 }}>
            ...
            <button style={S.btn(sec.color)} onClick={() => {
              if (qIdx < sec.questions.length - 1) setQIdx((p) => p + 1);
              else endSection(secIdx, answers, secResults);
            }}>
        ...
        {/* Palette sidebar */}
        <div style={{ width: 200, background: "#0d1117", borderLeft: "1px solid rgba(255,255,255,.07)", padding: "18px 14px", overflowY: "auto", flexShrink: 0 }}>
          <div style={{ fontSize: 9, letterSpacing: 2, color: "rgba(255,255,255,.25)", marginBottom: 14 }}>QUESTION PALETTE</div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 5 }}>
            {sec.questions.map((_, i) => {
              const k = key(i);
              const isCur = i === qIdx, isAns = answers[k] !== undefined, isFlag = flagged[k];
              return (
                <button key={i} onClick={() => setQIdx(i)} style={{ aspectRatio: 1, border: `2px solid ${isCur ? sec.color : isFlag ? "#FF6B6B" : isAns ? "#6BCB77" : "rgba(255,255,255,.1)"}`, background: isCur ? `${sec.color}20` : isFlag ? "rgba(255,107,107,.1)" : isAns ? "rgba(107,203,119,.1)" : "transparent", borderRadius: 4, color: isCur ? sec.color : isFlag ? "#FF6B6B" : isAns ? "#6BCB77" : "rgba(255,255,255,.35)", fontSize: 10, fontWeight: 700, cursor: "pointer", fontFamily: "'IBM Plex Mono','Courier New',monospace" }}>{i + 1}</button>
              );
            })}
          </div>
          <div style={{ marginTop: 20, fontSize: 10, color: "rgba(255,255,255,.25)", lineHeight: 2.2 }}>
          ...
          <div style={{ marginTop: 20 }}>
            <div style={{ fontSize: 9, letterSpacing: 2, color: "rgba(255,255,255,.25)", marginBottom: 10 }}>SECTIONS</div>
            {SECTIONS.map((s, i) => (
              <div key={s.id} style={{ padding: "5px 8px", borderRadius: 4, marginBottom: 3, fontSize: 10, color: i < secIdx ? "rgba(255,255,255,.25)" : i === secIdx ? s.color : "rgba(255,255,255,.2)", background: i === secIdx ? `${s.color}12` : "transparent" }}>
    ...
    <div style={{ ...S.page, padding: "40px 20px" }}>
      <div style={{ maxWidth: 880, margin: "0 auto" }}>
        <div style={{ textAlign: "center", marginBottom: 44 }}>
          <div style={{ fontSize: 10, color: "rgba(255,255,255,.3)", letterSpacing: 4, marginBottom: 14 }}>TEST COMPLETED</div>
          <h1 style={{ fontSize: "clamp(28px,6vw,52px)", fontWeight: 900, margin: "0 0 6px" }}>{candidate.name}</h1>
          <div style={{ fontSize: 12, color: "rgba(255,255,255,.35)", letterSpacing: 2 }}>{new Date().toLocaleDateString("en-IN", { day: "2-digit", month: "long", year: "numeric" })}</div>
          {saving && <div style={{ marginTop: 10, fontSize: 11, color: "rgba(255,255,255,.3)" }}>Saving to leaderboard...</div>}
        ...
        <div style={{ ...S.card, border: `1px solid ${g.color}40`, textAlign: "center", padding: "44px 32px", marginBottom: 28, position: "relative", overflow: "hidden" }}>
          ...
          <div style={{ position: "relative", zIndex: 1 }}>
            ...
            <div style={{ fontSize: 16, color: "rgba(255,255,255,.5)", marginBottom: 16 }}>Overall Score</div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 24, maxWidth: 420, margin: "0 auto" }}>
              {[{ v: `${correct}/${total}`, l: "Correct" }, { v: attempted, l: "Attempted" }, { v: total - attempted, l: "Skipped" }].map((i) => (
                <div key={i.l}>
                  ...
                  <div style={{ fontSize: 10, color: "rgba(255,255,255,.35)", letterSpacing: 2 }}>{i.l.toUpperCase()}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        <div style={{ marginBottom: 36 }}>
          <div style={{ fontSize: 10, color: "rgba(255,255,255,.3)", letterSpacing: 3, marginBottom: 16 }}>SECTION BREAKDOWN</div>
          <div style={{ display: "grid", gap: 10 }}>
            {SECTIONS.map((sec) => {
              const r = secResults[sec.id]; if (!r) return null;
              ...
              return (
                <div key={sec.id} style={{ ...S.card, display: "flex", alignItems: "center", gap: 20 }}>
                  ...
                  <div style={{ textAlign: "right", flexShrink: 0 }}>
                    ...
                    <div style={{ fontSize: 10, color: "rgba(255,255,255,.35)" }}>{r.correct}/{r.total}</div>
                  </div>
                </div>
              );
            })}
        ...
        <div style={{ marginBottom: 40 }}>
          <div style={{ fontSize: 10, color: "rgba(255,255,255,.3)", letterSpacing: 3, marginBottom: 16 }}>ANSWER REVIEW</div>
          {SECTIONS.map((sec) => (
            <div key={sec.id} style={{ marginBottom: 28 }}>
              ...
              <div style={{ display: "grid", gap: 8 }}>
                {sec.questions.map((q, i) => {
                  const k = `${sec.id}-${i}`;
                  const sel = answers[k];
                  const isAtt = sel !== undefined, isCor = sel === q.answer;
                  return (
                    <div key={i} style={{ background: "rgba(255,255,255,.02)", border: `1px solid ${!isAtt ? "rgba(255,255,255,.06)" : isCor ? "rgba(107,203,119,.2)" : "rgba(255,107,107,.2)"}`, borderLeft: `4px solid ${!isAtt ? "rgba(255,255,255,.1)" : isCor ? "#6BCB77" : "#FF6B6B"}`, borderRadius: 8, padding: "12px 18px" }}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12 }}>
                        <div style={{ fontSize: 12, color: "rgba(255,255,255,.65)", flex: 1, lineHeight: 1.6 }}>
                          <strong style={{ color: "rgba(255,255,255,.4)" }}>Q{i + 1}. </strong>
                          {q.text.split("\n")[0].substring(0, 90)}{q.text.length > 90 ? "…" : ""}
                        </div>
                        <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: 1, flexShrink: 0, padding: "2px 10px", borderRadius: 10, background: !isAtt ? "rgba(255,255,255,.05)" : isCor ? "rgba(107,203,119,.15)" : "rgba(255,107,107,.15)", color: !isAtt ? "rgba(255,255,255,.3)" : isCor ? "#6BCB77" : "#FF6B6B" }}>
                          {!isAtt ? "SKIPPED" : isCor ? "✓ CORRECT" : "✗ WRONG"}
                        </div>
                      </div>
                      {isAtt && !isCor && (
                        <div style={{ marginTop: 6, fontSize: 11, color: "rgba(107,203,119,.8)" }}>
        ...
        <div style={{ ...S.card, border: "1px solid rgba(0,212,255,.2)", textAlign: "center", padding: "28px", marginBottom: 32 }}>
          <div style={{ fontSize: 20, marginBottom: 8 }}>📢 Share Your Score!</div>
          <p style={{ fontSize: 13, color: "rgba(255,255,255,.5)", margin: "0 0 16px" }}>Share on WhatsApp and challenge friends to beat your score!</p>
          <button onClick={() => {
            const text = `🎯 I scored ${pct}% (${correct}/${total}) on the TCS NQT Mock Test!\nGrade: ${g.g} — ${g.label}\n\nTry it free 👇\n${window.location.href}`;
            window.open(`https://wa.me/?text=${encodeURIComponent(text)}`, "_blank");
          }} style={{ padding: "13px 32px", background: "#25D366", border: "none", borderRadius: 5, color: "#fff", fontWeight: 700, fontSize: 12, letterSpacing: 2, cursor: "pointer", fontFamily: "'IBM Plex Mono','Courier New',monospace" }}>
            📱 SHARE ON WHATSAPP
          </button>
        </div>
        ...
        <p style={{ textAlign: "center", marginTop: 40, fontSize: 10, color: "rgba(255,255,255,.15)", letterSpacing: 1 }}>Practice makes perfect. Keep attempting mock tests to ace TCS NQT!</p>
      </div>
    </div>
  );
}
...
export default function App() {
  ...
  const handleRetake   = ()        => { setSecResults({}); setAnswers({}); setCandidate(null); setScreen("home"); };
  ...
  if (screen === "test")         return <TestScreen candidate={candidate} onComplete={(r) => { handleComplete(r); }} />;
